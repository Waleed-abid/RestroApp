...
const order = (prev, curr) => {
  if (curr.offer_category_id == 1) {
    ...
    prev["sub_total"] +=
      (100 / (100 + curr?.vat * 100)) * curr?.price * curr.quantity;
    prev["tax"] +=
      (curr?.price - (100 / (100 + curr?.vat * 100)) * curr?.price) *
      curr?.quantity;
  ...
  if (curr.offer_category_id == 2) {
    ...
    prev["sub_total"] +=
      curr?.discount?.temp_price != 0
        ? curr?.discount?.temp_price *
          (100 / (100 + curr?.vat * 100)) *
          curr?.quantity
        : (100 / (100 + curr?.vat * 100)) *
          (curr?.price - curr?.price * curr?.discount?.discount) *
          curr?.quantity;
    prev["tax"] +=
      curr?.discount?.temp_price != 0
        ? (curr?.discount?.temp_price -
            (100 / (100 + curr?.vat * 100)) * curr?.discount?.temp_price) *
          curr?.quantity
        : (curr?.price - (100 / (100 + curr?.vat * 100)) * curr?.price) *
          curr?.quantity;
    prev.line_items.push(curr);
...
module.exports = (line_items) => {
  return line_items?.reduce(
    ...
    { total: 0, tax: 0, discount: 0, sub_total: 0, line_items: [] }
  );
};
